...
def relabel_nodes(G, max_nodes):
    ...
    lin_centrality_attrs = np.linspace(-1, 1, max_nodes, endpoint=True) # relabel nodes depending on centrality encoding
    mapping = {}
...
def constGraphList(matrix, minNodes, maxNodesT, maxNodesQ=None, use_prcnt=10, idx_list=None):
...
def choose_graph(graphs):
    ...
    graph = to_networkx(
            random.choice(graphs), to_undirected=True, node_attrs=["x"], edge_attrs=["edge_attr"]
        )
    return graph
...
def synthesize_graphs(graphs, cat, n, min_size, max_size_Q, max_size_T, margin):
    ...
    while len(a_) < n:
        ...
        if cat == 0: # graphs sampled from same region
        ...
        elif cat == 1: # graphs sampled from same region, edge feats changed within margin
            graph_tup = sample_same_region(graphs, min_size, max_size_T, a_uniq, b_uniq)
            if graph_tup == None:
                continue
            else:
                ...
                for edge, update in zip(list(b_graph.edges()), edge_updates):
                    ...
                    b_graph[edge[0]][edge[1]]["edge_attr"] = np.clip(val + update, 10e-6, 1)
            ...
            graph_pair = sample_different_region(graphs, min_size, max_size_T, a_uniq, b_uniq)
            if graph_pair == None:
                continue
            else:
        ...
        elif cat == 3: # graphs sampled from same region, edge feats changed outside margin
            graph_tup = sample_same_region(graphs, min_size, max_size_T, a_uniq, b_uniq)
            if graph_tup == None:
                continue
            else:
            ...
            if graph_tup == None:
                ...
                a_graph, b_graph, _ = graph_tup
                _, added_edges = add_and_remove_edges(
                    b_graph, random.random(), 0
                )
            ...
            if graph_tup == None:
                ...
                nodes_to_add = np.random.choice(poss_nodes, nNodes_to_add, replace=False)
